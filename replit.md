...
- **Mobile**: **MAJOR ARCHITECTURAL CHANGE (Nov 1, 2025)**: Converted from full React Native app to WebView wrapper architecture (v2.0.0). The mobile app is now a native container that loads the existing web application (https://www.mindmymoneyapp.com) inside a WebView. This approach provides: (1) Real downloadable App Store app with native app icon and navigation, (2) Automatic feature parity with web app - any web updates appear in mobile instantly, (3) Native cookie-based authentication - eliminates previous session management issues, (4) App Store compliance - Stripe payments open in Safari instead of in-app, (5) Significantly reduced maintenance - one codebase (web) powers both platforms. The app includes a native navigation bar with back/forward/refresh buttons, automatic Stripe checkout detection (opens Safari), pull-to-refresh functionality, and comprehensive error handling. Previous React Native screens preserved as backup in `src/screens/` but no longer imported. Users can sign up, log in, and use all features directly within the WebView using the existing web interface. See `mobile/WEBVIEW_README.md` for complete documentation.
