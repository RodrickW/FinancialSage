...
- **Interview-as-Onboarding (Mar 2026)**: New users are now redirected to the Money Mind Coach Interview immediately after registration. Free-tier users can complete the full interview with responses saved via `POST /api/interview/save` (no AI generation); they see an upgrade prompt at completion. Plus/Pro users get the full AI-generated personalized plan. A "Skip for now" button lets users bypass the interview.
- **What If Financial Simulator (Mar 2026)**: New page at `/what-if`. Three sliders (extra savings, extra debt payment, spending reduction) drive a live 5-year projection area chart (Baseline vs Optimized), debt payoff impact, and real-time summary cards.
- **Debt Payoff Visualizer (Mar 2026)**: New page at `/debt-payoff`. Supports Snowball and Avalanche strategies with editable debt entries, extra payment slider, declining balance bar chart, and summary showing months to debt-free and interest saved.
- **Money Story Monthly Recap (Mar 2026)**: New page at `/money-story`. Shareable monthly financial recap card with letter grade, spending breakdown donut chart, budget health tracker, and share/save functionality.
- **Faith Mode Full Experience (Mar 2026)**: New page at `/faith-mode`, gated by `faithModeEnabled` user flag (set in Settings). Displays daily rotating scripture (12 verses keyed by day of year), Generosity Score with letter grade, tithing calculator with adjustable percentage, and generosity tips.
- **Financial Tools Dashboard Section (Mar 2026)**: Dashboard now shows a "Financial Tools" 2-column card grid linking to all four new tool pages. Faith Mode card only appears when the user has Faith Mode enabled.
- **iOS App Store Rejection Fixes (Apr 2026)**: Fixed multiple bugs causing Apple review failures: (1) Blank interview screen — removed tier gate blocking all users from `/coach/interview`. (2) Playbook locked after purchase — added `POST /api/ai/interview/regenerate` endpoint and "Generate My Playbook" button. (3) PaywallScreen pricing bug — fixed `getPackageForSelection()` fallback. (4) Post-purchase APIs still blocked — added `POST /api/mobile/sync-subscription` that verifies purchase with RevenueCat and updates DB immediately; MobileUserSync.tsx polls for native tier and calls it. (5) Daily check-in sending users back to interview when they already have one — backend now returns `needsPlaybook` vs `needsInterview` separately; DailyCheckin.tsx shows the correct CTA for each. (6) Demo account playbook generation failing — demo setup seeds a hardcoded interview + playbook for Alex Demo so no OpenAI call is needed during Apple review. Demo setup endpoint: `POST /api/demo/setup`.
