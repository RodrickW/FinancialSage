...
- **iOS App Store Rejection Fixes (Apr 2026)**: Fixed three bugs causing Apple review failures: (1) Blank interview screen — removed tier gate blocking all users from `/coach/interview`; all users can now take the interview regardless of tier (AI generation still gated to Plus/Pro at the end). (2) Playbook locked after purchase — added `POST /api/ai/interview/regenerate` endpoint and "Generate My Playbook" button on the Playbook page for users who completed the interview as free tier then upgraded. (3) PaywallScreen pricing bug — fixed `getPackageForSelection()` fallback that was showing Plus price ($5.99) on the Pro tab; now falls back to hardcoded FALLBACK_PRICES when no tier-matching package is found. Also fixed interview mutation to check `window.mobileSubscriptionTier` (native Apple IAP tier) so AI generation triggers immediately after purchase without waiting for the RevenueCat webhook.
